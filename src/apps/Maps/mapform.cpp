...
void MapForm::wheelEvent(QWheelEvent* aEvent)
    ...
    if (r.Contains(p))
        {
        m_framework->ConvertPoint(p,CartoType::TCoordType::Display,CartoType::TCoordType::Map);
        set_centre = true;
        }
    ...
    */
    ...
    if (set_centre)
        m_framework->Pan(p.iX,p.iY,CartoType::TCoordType::Map,point.x(),point.y(),CartoType::TCoordType::Display);
...
void MapForm::ShowNextFoundObject()
    ...
    CartoType::TResult error = m_framework->InsertCopyOfMapObject(memory_map_handle,found_layer,*object,
                                                                  object->Type() == CartoType::TMapObjectType::Point ? 50 : 0,
