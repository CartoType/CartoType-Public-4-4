...
class TContour: public MPath
    ...
    bool ClockwiseOrientation() const;
    bool VertexClockwiseOrientation(size_t aIndex) const;
    bool Contains(double aX,double aY) const;
    bool Contains(const TPoint& aPoint) const { return Contains(aPoint.iX,aPoint.iY); }
    const MPath* ClippedContour(const TRect& aClip,bool aAlwaysCreateNewPath = false) const;
    ...
    */
    template<typename MTraverser> void Traverse(MTraverser& aTraverser,const TRect* aClip = nullptr) const
        ...
        if (aClip && !aClip->IsMaximal())
            {
            const MPath* clipped_contour = ClippedContour(*aClip);
            if (clipped_contour != this)
                {
                clipped_contour->Traverse(aTraverser);
                delete (MPath*)clipped_contour;
                return;
                }
            }
...
class CGeometry
    ...
    void AppendPoint(TPointFP aPoint);
    void BeginContour();
