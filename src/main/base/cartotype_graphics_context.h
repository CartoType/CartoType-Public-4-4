...
class TTextParam
    ...
    */
    ...
    /** A flag used in iFlags: if drawing text on a path, traverse the path backwards. */
    static constexpr uint32 KTraversePathBackwards = 1;
    /**
    A flag used in iFlags: if drawing text on a path, stretch it out to fit the path 
    by adding letter spacing and word spacing.
    */
    static constexpr uint32 KCopyFit = 2;
    /**
    A flag used in iFlags: round advances to integer values, which is normally
    done if drawing horizontally or vertically, to maximise glyph cache hits.
    */
    static constexpr uint32 KRoundAdvances = 4;
